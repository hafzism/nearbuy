...
class MyEdit extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const dboy_edit_profile(title: 'Edit Profile'),
    );
  }
}

class dboy_edit_profile extends StatefulWidget {
  const dboy_edit_profile({super.key, required this.title});

  final String title;

  @override
  State<dboy_edit_profile> createState() => _dboy_edit_profileState();
}

class _dboy_edit_profileState extends State<dboy_edit_profile> {
  _dboy_edit_profileState() {
    _get_data();
  }
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'ok') {
          setState(() {
            ...
            p1 = '${sh.getString("img_url")}${data['photo']}';
            p2 = '${sh.getString("img_url")}${data['photo1']}';
          });
        } else {
            ...
            children: <Widget>[
              if (_selectedImage != null) ...{
                ...
                InkWell(
                  ...
                  child: Column(
                    children: [
                      Image.network(p1, height: 200, width: 200),
              ...
              if (_selectedImage2 != null) ...{
                ...
                InkWell(
                  ...
                  child: Column(
                    children: [
                      Image.network(p2, height: 200, width: 200),
                      const Text('Select Image', style: TextStyle(color: Colors.cyan))
                    ],
                  ),
                ),
  ...
  void senddata() async {
    ...
    final urls = Uri.parse('$url/dboy_edit_profile/');
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status == 'ok') {
          ...
          {Navigator.push(context, MaterialPageRoute(builder: (context)=>dboy_vprofile(title: "",),),);}// Navigate to the next screen
        } else {
